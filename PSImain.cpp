...
//      "synthetic/synthetic_5d.neighbors"
        folder + "z0.0.points",
        folder + "z0.0.tris",
        folder + "z0.0.neighbors",
        0
//          folder + "tempdata.csv",
//          folder + "tempdata.tris",
//          folder + "tempdata.neighbors",
//          1
  );
...
//    target[5] = 18.5 + float(rand()) / RAND_MAX * 4;
    ...
    interpolations[i] = cool->interpolate(target);
  }
  ...
  double * qualities = cool->qualities;
  double q_mean = 0;
  double i_mean = 0;
  double q_M2 = 0;
  double i_M2 = 0;
  ...
  double i_max = 0;
  std::ofstream q_file;
  q_file.open(folder + "dip_qualities.csv");
  q_file << "qual,dip_interp,real_val\n";
...
void test_projective_simplex(int adaptive) {
  ...
  std::string folder = "psi_19/";
  std::string fname = "z0.0.points";
  psi_read_points(folder + fname, 0);
//  std::string fname = "tempdata.csv";
//  psi_read_points(folder + fname, 1);
...
//          std::cout << interpolations[i] << "+=" << bary[j] << " * " << sobj.points[j]->value << std::endl;
  ...
  double i_max = 0;
  std::ofstream q_file;
  q_file.open(folder + "psi_qualities.csv");
  q_file << "qual,psi_interp,real_val\n";
